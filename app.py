...
with st.sidebar:
    ...
    st.divider()
    page = st.radio("Navigate", ["Home", "Rankings", "Competitions", "Venues", "Graphs"],
...
# =====================================================================
if page == "Home":
    st.markdown('<div class="hero-title">Tennis Rankings and Game Analytics</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="hero-subtitle">Interactive insights on player rankings, competition structures, and venue data powered by Sportradar API.', unsafe_allow_html=True
    )

    total_competitors = run_query("SELECT COUNT(*) AS n FROM competitors")["n"][0]
    total_countries = run_query("SELECT COUNT(DISTINCT country) AS n FROM competitors")["n"][0]
    max_points = int(run_query("SELECT MAX(points) AS n FROM competitor_rankings")["n"][0])
    ...
    with st.container(border=True):
        eyebrow("BY COUNTRY")
        section_header("Country-Wise Analysis", "Total competitors and their average ranking points, by country.")
        ...
        if search_country:
            country_df = country_df[country_df["country"].str.contains(search_country, case=False, na=False)]
        st.dataframe(country_df, use_container_width=True, hide_index=True, height=280)
        download_button(country_df, "Download as CSV", "country_analysis.csv")

    # --- Leaderboards ----------------------------------------------------
    with st.container(border=True):
        eyebrow("LEADERBOARDS")
        section_header("Top Performers", "The best-ranked competitors, and the highest point totals on tour.")
        col_a, col_b = st.columns(2)
        with col_a:
            st.markdown('<div class="card-title">Top-Ranked Competitors</div>', unsafe_allow_html=True)
        ...
        with col_b:
            st.markdown('<div class="card-title">Highest Points</div>', unsafe_allow_html=True)
...
# =====================================================================
    ...
    st.markdown('<div class="hero-title">Rankings</div>', unsafe_allow_html=True)
    st.markdown(
    ...
    # Callback function to clear all filter keys
    ...
    with st.container(border=True):
        # Header row with section info and Reset Button
...
# =====================================================================
    ...
    st.markdown('<div class="hero-title">Competitions</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">Browse every tracked competition across all tours and categories.</div>',
                unsafe_allow_html=True)
    ...
    with st.container(border=True):
        eyebrow("SEARCH & FILTER")
        section_header("All Competitions", "Search by name, or narrow down by type and category.")

        c1, c2, c3 = st.columns(3)
        search_comp = c1.text_input("Search competition name")
        type_filter = c2.multiselect("Type", options=sorted(comps["type"].unique()))
        category_filter = c3.multiselect("Category", options=sorted(comps["category_name"].unique()))

        display_comps = comps.copy()
        if search_comp:
            display_comps = display_comps[display_comps["competition_name"].str.contains(search_comp, case=False, na=False)]
        if type_filter:
            display_comps = display_comps[display_comps["type"].isin(type_filter)]
        if category_filter:
            display_comps = display_comps[display_comps["category_name"].isin(category_filter)]

        st.dataframe(display_comps, use_container_width=True, hide_index=True, height=320)
        download_button(display_comps, "Download filtered results", "filtered_competitions.csv")

# =====================================================================
# VENUES
# =====================================================================
elif page == "Venues":
    st.markdown('<div class="hero-title">Venues & Complexes</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">Every tournament venue, grouped by complex and country.</div>',
                unsafe_allow_html=True)
    ...
    with st.container(border=True):
        eyebrow("SEARCH & FILTER")
        section_header("All Venues", "Search by venue name, or filter by country.")

        c1, c2 = st.columns(2)
        search_venue = c1.text_input("Search venue name")
        countries = ["All"] + sorted(venues["country_name"].dropna().unique().tolist())
        country_filter = c2.selectbox("Country", countries)

        display_venues = venues.copy()
        if search_venue:
            display_venues = display_venues[display_venues["venue_name"].str.contains(search_venue, case=False, na=False)]
        if country_filter != "All":
            display_venues = display_venues[display_venues["country_name"] == country_filter]

        st.dataframe(display_venues, use_container_width=True, hide_index=True, height=320)
        download_button(display_venues, "Download filtered results", "filtered_venues.csv")

# =====================================================================
# GRAPHS — all 14 charts
# =====================================================================
elif page == "Graphs":
    st.markdown('<div class="hero-title">Insight Graphs</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">All 14 exploratory charts from the analysis notebook — '
                'rendered independently of the dashboard theme to preserve the original output.</div>',
                unsafe_allow_html=True)
    ...
    charts = [
        ...
        """, "bar", "category_name", "competition_count", "seagreen"),
        ...
        """, "barh", "country", "competitor_count", "steelblue"),
        ...
        """, "bar", "level", "competition_count", "mediumpurple"),
        ...
        """, "bar", "complex_name", "venue_count", "teal"),
        ...
        """, "barh", "country_name", "venue_count", "indianred"),
        ...
        """, "bar", "timezone", "venue_count", "goldenrod"),
        ...
        """, "bar", "country", "avg_points", "crimson"),
        ...
        """, "bar", "category_name", "doubles_count", "cornflowerblue"),
    ]
    ...
    # --- Pie / grouped / histogram charts (custom shapes) -----------------
    pie_col1, pie_col2 = st.columns(2)

    with pie_col1:
        with st.container(border=True):
            st.markdown('<div class="card-title">Rank Movement Summary</div>', unsafe_allow_html=True)
            df = run_query("""
            ...
            ax.pie(df["competitor_count"], labels=df["movement_status"], autopct="%1.1f%%",
                   colors=["mediumseagreen", "tomato", "lightgray"])
            st.pyplot(fig)

    with pie_col2:
        with st.container(border=True):
            st.markdown('<div class="card-title">Top-Level vs Sub-Competitions</div>', unsafe_allow_html=True)
            df = run_query("""
            ...
            """)
            fig, ax = plt.subplots(figsize=(5, 5))
            ax.pie(df["total"], labels=df["level_type"], autopct="%1.1f%%",
    ...
    with pie_col3:
        with st.container(border=True):
            st.markdown('<div class="card-title">Competitions by Gender</div>', unsafe_allow_html=True)
            df = run_query("SELECT gender, COUNT(*) AS total FROM competitions GROUP BY gender")
            fig, ax = plt.subplots(figsize=(5, 5))
            ax.pie(df["total"], labels=df["gender"], autopct="%1.1f%%",
                   colors=["skyblue", "lightpink", "khaki"])
            st.pyplot(fig)

    with pie_col4:
        with st.container(border=True):
            st.markdown('<div class="card-title">Distribution of Ranking Points</div>', unsafe_allow_html=True)
            df = run_query("SELECT points FROM competitor_rankings")
            fig, ax = plt.subplots(figsize=(6, 5))
            ax.hist(df["points"], bins=20, color="mediumslateblue", edgecolor="white")
            st.pyplot(fig)

    with st.container(border=True):
        st.markdown('<div class="card-title">Competition Types by Category</div>', unsafe_allow_html=True)
        df = run_query("""
            SELECT cat.category_name, c.type, COUNT(*) AS total
            FROM competitions c JOIN categories cat ON c.category_id = cat.category_id
            GROUP BY cat.category_name, c.type
        """)
        pivot = df.pivot(index="category_name", columns="type", values="total").fillna(0)
